...
public class AutoFarmMod extends JavaPlugin {
    ...
    public void setup() {
        ...
        try {
            ...
            // 3. Register Hytale Built-in 'F' Key Custom UI Interaction
            try {
                com.hypixel.hytale.server.core.modules.interaction.interaction.config.server.OpenCustomUIInteraction.registerCustomPageSupplier(
                        ...
                        (playerEntityRef, accessor, playerRef, ctx) -> {
                            System.out.println("[AutoFarmMod] 'AutoFarm_Config' CustomPageSupplier invoked by " + (playerRef != null ? playerRef.getUsername() : "player"));
                            Vector3i pos = null;
            ...
            System.out.println("[AutoFarmMod] Registered Placement/Destruction Handlers, FarmScanSystem, and UI Handlers.");
        } catch (Exception e) {
    ...
    private void onPlayerInteract(com.hypixel.hytale.server.core.event.events.player.PlayerInteractEvent event) {
        ...
        if (farm == null) {
            ...
            if (blockName != null && (blockName.toLowerCase(java.util.Locale.ROOT).contains("autofarm") || blockName.equalsIgnoreCase("autofarm_block"))) {
        ...
        if (farm != null) {
            ...
            System.out.println("[AutoFarmMod] Player interacted with AutoFarm block at " + targetBlock + " | Action: " + action);
